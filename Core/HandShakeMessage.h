...
public:
	...
	HandShakeMessage(string playerName, string hashedPassword, bool spectator, uint32_t emuVersion) : NetMessage(
	...
	static string GetPasswordHash(string serverPassword, string connectionHash)
	{
		...
		vector<uint8_t> dataToHash = vector<uint8_t>(saltedPassword.c_str(),
		                                             saltedPassword.c_str() + saltedPassword.size());
