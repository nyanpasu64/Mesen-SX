...
void Msu1::Write(uint16_t addr, uint8_t value)
{
	switch (addr)
	{
	case 0x2000: _tmpDataPointer = (_tmpDataPointer & 0xFFFFFF00) | value;
		break;
	case 0x2001: _tmpDataPointer = (_tmpDataPointer & 0xFFFF00FF) | (value << 8);
		break;
	case 0x2002: _tmpDataPointer = (_tmpDataPointer & 0xFF00FFFF) | (value << 16);
		break;
	case 0x2003:
	...
	case 0x2004: _trackSelect = (_trackSelect & 0xFF00) | value;
		break;
	case 0x2005:
...
void Msu1::Serialize(Serializer& s)
{
	...
	s.Stream(_trackSelect, _tmpDataPointer, _dataPointer, _repeat, _paused, _volume, _trackMissing, _audioBusy,
	         _dataBusy, offset);
	if (!s.IsSaving())
	{
		_dataFile.seekg(_dataPointer, ios::beg);
