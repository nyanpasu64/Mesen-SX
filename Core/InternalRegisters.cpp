	...
	case 0x4212:
		{
			...
			return (
				...
				((_state.EnableAutoJoypadRead && scanline >= nmiScanline && scanline <= nmiScanline + 2) ? 0x01 : 0) |
				//Auto joypad read in progress
...
void InternalRegisters::Write(uint16_t addr, uint8_t value)
{
	...
	case 0x420D: _state.EnableFastRom = (value & 0x01) != 0;
		break;
...
{
	s.Stream(
		_state.EnableFastRom, _nmiFlag, _state.EnableNmi, _state.EnableHorizontalIrq, _state.EnableVerticalIrq,
		_state.HorizontalTimer,
		_state.VerticalTimer, _state.IoPortOutput, _state.ControllerData[0], _state.ControllerData[1],
		_state.ControllerData[2], _state.ControllerData[3],
