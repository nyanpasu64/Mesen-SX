...
uint8_t PpuTools::GetTilePixelColor(const uint8_t* ram, const uint32_t ramMask, const uint8_t bpp,
                                    const uint32_t pixelStart, const uint8_t shift)
{
	uint8_t color;
	if (bpp == 2)
	{
...
uint32_t PpuTools::GetRgbPixelColor(uint8_t* cgram, uint8_t colorIndex, uint8_t palette, uint8_t bpp,
                                    bool directColorMode, uint16_t basePaletteOffset)
{
...
void PpuTools::GetTileView(GetTileViewOptions options, uint8_t* source, uint32_t srcSize, uint8_t* cgram,
                           uint32_t* outBuffer)
{
	...
	{
	case TileFormat::Bpp2: bpp = 2;
		break;
	case TileFormat::Bpp4: bpp = 4;
		break;
	case TileFormat::DirectColor: directColor = true;
		bpp = 8;
		break;
	case TileFormat::Mode7: bpp = 16;
		break;
	case TileFormat::Mode7DirectColor: directColor = true;
		bpp = 16;
		break;
	default: bpp = 8;
		break;
	...
	{
	case TileBackground::Default: bgColor = DefaultVideoFilter::ToArgb((cgram[1] << 8) | cgram[0]);
		break;
	case TileBackground::PaletteColor: bgColor = DefaultVideoFilter::ToArgb(0);
		break;
	case TileBackground::Black: bgColor = DefaultVideoFilter::ToArgb(0);
		break;
	case TileBackground::White: bgColor = DefaultVideoFilter::ToArgb(0x7FFF);
		break;
	case TileBackground::Magenta: bgColor = DefaultVideoFilter::ToArgb(0x7C1F);
		break;
	}
	...
	for (int row = 0; row < rowCount; row++)
	{
		...
		for (int column = 0; column < options.Width; column++)
		{
			...
			{
				int displayColumn = (column / 2) + (column & 0x01) + ((row & 0x01) ? options.Width / 2 : 0) + (
					(column & 0x02) ? -1 : 0);
				...
				for (int y = 0; y < 8; y++)
				{
					...
					for (int x = 0; x < 8; x++)
					{
						...
						if (color != 0 || options.Background == TileBackground::PaletteColor)
						{
							...
							if (directColor)
							{
								rgbColor = DefaultVideoFilter::ToArgb(
									((color & 0x07) << 2) | ((color & 0x38) << 4) | ((color & 0xC0) << 7));
							}
							else
							{
			...
			else
			{
				for (int y = 0; y < 8; y++)
				{
					...
					for (int x = 0; x < 8; x++)
					{
						...
						if (color != 0 || options.Background == TileBackground::PaletteColor)
						{
							outBuffer[baseOutputOffset + (y * options.Width * 8) + x] = GetRgbPixelColor(
								cgram, color, options.Palette, bpp, directColor, 0);
						}
					}
				}
			}
				...
				for (int y = 0; y < tileHeight; y++)
				{
					...
					for (int x = 0; x < tileWidth; x++)
					{
						...
						if (color != 0)
						{
							...
							outBuffer[((row * tileHeight) + y) * 1024 + column * tileWidth + x] = GetRgbPixelColor(
								cgram, color, palette, bpp, directColor, basePaletteOffset);
						}
					}
				}
			}
...
void PpuTools::GetSpritePreview(GetSpritePreviewOptions options, PpuState state, uint8_t* vram, uint8_t* oamRam,
	...
	for (int screenY = 0; screenY < lastScanline; screenY++)
	{
		for (int i = 508; i >= 0; i -= 4)
		{
			...
			for (int j = std::max<int16_t>(x, 0); j < x + width && j < 256; j++)
			{
				...
				uint16_t tileStart = ((state.OamBaseAddress + (tileIndex << 4) + (useSecondTable
					                                                                  ? state.OamAddressOffset
					                                                                  : 0)) & 0x7FFF) << 1;
		...
		for (int column = 0; column < 32; column++)
		{
			...
			for (int y = 0; y < 8; y++)
			{
				...
				for (int x = 0; x < 8; x++)
				{
					...
					outBuffer[((row * 8) + y) * 1024 + column * 8 + x] = DefaultVideoFilter::ToArgb(
						state.CgbBgPalettes[bgPalette + color]);
				}
			}
		}
	}
...
void PpuTools::GetGameboySpritePreview(GetSpritePreviewOptions options, GbPpuState state, uint8_t* vram,
                                       uint8_t* oamRam, uint32_t* outBuffer)
{
