...
void VideoDecoder::UpdateVideoFilter()
{
	...
	if (_videoFilterType != newFilter || _videoFilter == nullptr)
	{
		...
		case VideoFilterType::NTSC: _videoFilter.reset(new NtscFilter(_console));
			break;
		default: _scaleFilter = ScaleFilter::GetScaleFilter(_videoFilterType);
			break;
		}
	}
}
...
void VideoDecoder::DecodeFrame(bool forRewind)
{
	...
	if (_previousScale != config.VideoScale || screenSize.Height != _previousScreenSize.Height || screenSize.Width !=
		_previousScreenSize.Width)
	{
