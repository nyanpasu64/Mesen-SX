...
void NtscFilter::OnBeforeApplyFilter()
{
	...
	if (_ntscSetup.hue != cfg.Hue / 100.0 || _ntscSetup.saturation != cfg.Saturation / 100.0 || _ntscSetup.brightness !=
		cfg.Brightness / 100.0 || _ntscSetup.contrast != cfg.Contrast / 100.0 ||
		_ntscSetup.artifacts != cfg.NtscArtifacts || _ntscSetup.bleed != cfg.NtscBleed || _ntscSetup.fringing != cfg.
		NtscFringing || _ntscSetup.gamma != cfg.NtscGamma ||
		(_ntscSetup.merge_fields == 1) != cfg.NtscMergeFields || _ntscSetup.resolution != cfg.NtscResolution || _ntscSetup
		.sharpness != cfg.NtscSharpness)
	{
...
void NtscFilter::ApplyFilter(uint16_t* ppuOutputBuffer)
{
	...
	if (useHighResOutput)
	{
		snes_ntsc_blit_hires(&_ntscData, ppuOutputBuffer, 512, IsOddFrame() ? 0 : 1, 512, _baseFrameInfo.Height,
		                     _ntscBuffer, SNES_NTSC_OUT_WIDTH(256) * 4);
	}
	else
	{
		snes_ntsc_blit(&_ntscData, ppuOutputBuffer, 256, IsOddFrame() ? 0 : 1, 256, _baseFrameInfo.Height, _ntscBuffer,
		               SNES_NTSC_OUT_WIDTH(256) * 8);
	}
	VideoConfig cfg = _console->GetSettings()->GetVideoConfig();
	...
	if (cfg.ScanlineIntensity == 0)
	{
		...
		{
			memcpy(GetOutputBuffer() + i * frameInfo.Width, _ntscBuffer + yOffset + xOffset + i * baseWidth,
			       frameInfo.Width * sizeof(uint32_t));
			memcpy(GetOutputBuffer() + (i + 1) * frameInfo.Width, _ntscBuffer + yOffset + xOffset + i * baseWidth,
			       frameInfo.Width * sizeof(uint32_t));
		}
	}
	else
