...
RegisterHandlerB::RegisterHandlerB(Console* console, Ppu* ppu, Spc* spc, uint8_t* workRam) : IMemoryHandler(
...
void RegisterHandlerB::Write(uint32_t addr, uint8_t value)
{
	...
	if (addr >= 0x2180 && addr <= 0x2183)
	{
		...
		case 0x2181: _wramPosition = (_wramPosition & 0x1FF00) | value;
			break;
		case 0x2182: _wramPosition = (_wramPosition & 0x100FF) | (value << 8);
			break;
		case 0x2183: _wramPosition = (_wramPosition & 0xFFFF) | ((value & 0x01) << 16);
			break;
		}
	}
